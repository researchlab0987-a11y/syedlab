...
export const createGalleryItem = onCall(async (request) => {
  ...
  if (!title || !imageUrl) {
    throw new HttpsError(
      "invalid-argument",
      "Title and imageUrl are required.",
    );
  }
...
export const notifyPrivateChatRecipient = onDocumentCreated(
  ...
  async (event) => {
    ...
    const sender =
      (senderSnap.data() as { name?: string; email?: string } | undefined) ??
      {};
    const recipient =
    ...
    const lastActiveMs =
      toMillis(presence.lastActiveAt) || toMillis(presence.lastSeenAt);
    ...
    const cooldownRef = db.doc(
      `privateChatNotifications/${recipientId}__${chatId}`,
    );
